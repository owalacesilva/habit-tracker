    ...
    <div className={cn('flex items-end justify-between gap-3', className)}>
      {items.map((item) => (
        <figure key={item.label} className="flex flex-1 flex-col items-center gap-3">
          <div
            className="relative h-56 w-full overflow-hidden rounded-pill bg-chart-track bg-[repeating-linear-gradient(45deg,rgba(255,255,255,.28)_0_6px,transparent_6px_12px)]"
            role="img"
            aria-label={format(labelTemplate, { label: item.label, percentage: item.percentage })}
          >
            <div
              className={cn(
          ...
          <figcaption className="w-full truncate text-center text-xs font-medium text-ink-muted">
            {item.label}
          </figcaption>
        </figure>
      ))}
